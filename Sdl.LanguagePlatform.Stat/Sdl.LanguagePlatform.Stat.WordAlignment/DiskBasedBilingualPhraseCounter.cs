...
{
	internal class DiskBasedBilingualPhraseCounter : IBilingualPhraseCounter
	{
		...
		public DiskBasedBilingualPhraseCounter(DataLocation location, CultureInfo srcCulture, CultureInfo trgCulture)
		{
		...
		private BilingualDictionaryFile GenerateBilingualDictionary(string sortedDataFileName, int minFreq)
		{
			PhraseDictionary phraseDictionary = new PhraseDictionary(_location, _sourceCulture);
			phraseDictionary.Load();
			PhraseDictionary phraseDictionary2 = new PhraseDictionary(_location, _targetCulture);
			phraseDictionary2.Load();
			BilingualDictionaryFile bilingualDictionaryFile = new BilingualDictionaryFile(_location, _sourceCulture, _targetCulture)
			{
		...
		{
			PhraseDictionary phraseDictionary = new PhraseDictionary(_location, culture);
			using (FileStream input = File.OpenRead(sortedDatafileName))
			{
				BinaryReader reader = new BinaryReader(input);
			...
			using (TokenFileReader tokenFileReader = new TokenFileReader(_location, culture))
			{
