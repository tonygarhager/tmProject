...
namespace Sdl.Enterprise2.MultiTerm.Client.IdentityModel
{
	internal class AuthorizationHelper
	{
		...
		{
			string cacheKey = identityInfo.CacheKey;
		...
		public void GetAllAuthorizationData()
		{
			...
			identityInfo.SetAllPermissions(allPermissions);
		}

		private string GetCurrentUser()
		{
			if (identityInfo.Credentials.UserType == UserManagerTokenType.WindowsUser)
			{
				return WindowsIdentity.GetCurrent()?.Name;
			}
			return identityInfo.Credentials.UserName;
		}
