...
namespace Sdl.Enterprise2.MultiTerm.Client.IdentityModel
{
	public abstract class IssuedTokenClientBase<T> : FaultExceptionHandlerBase, IDisposable where T : class
	{
		...
		public T GetProxy()
		{
			...
			if (ChannelFactory == null && (string.IsNullOrEmpty(baseAddress) || UseInlineInitialization(new Uri(baseAddress))))
			{
				proxy = InitializeInline();
				return proxy;
			}
		...
		protected virtual bool UseInlineInitialization(Uri serverAddress)
		{
			return serverAddress.DnsSafeHost.Equals("localhost", StringComparison.OrdinalIgnoreCase);
		}

		protected virtual T InitializeInline()
		{
			throw new NotImplementedException("Derived classes must override and implement this method.");
		}
