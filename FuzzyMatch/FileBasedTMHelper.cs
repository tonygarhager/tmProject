...
namespace concordance_search
{
    class FileBasedTMHelper
    {
        ...
        /// <param name="tmPath"></param>
        /// <param name="tmxFile"></param>
        public SearchResults ConcordanceSearch(string tmPath, bool target, string searchText, string maxResults, string minScore)
        {
            SearchResults results;
            FileBasedTranslationMemory tm = new FileBasedTranslationMemory(tmPath);
            results = tm.LanguageDirection.SearchText(GetSearchSettings(target, int.Parse(maxResults), int.Parse(minScore)), searchText);
            return results;
        }

        public SearchResults FuzzySearch(string tmPath, string query, string maxResults, string minScore)
        {
            SearchResults results;
            FileBasedTranslationMemory tm = new FileBasedTranslationMemory(tmPath);
            Segment orgSegment = new Segment(tm.LanguageDirection.SourceLanguage);
            orgSegment.Add(query);
            Segment tgtSegment = new Segment(tm.LanguageDirection.TargetLanguage);

            //results = tm.LanguageDirection.SearchSegment(GetSearchSettings(), orgSegment);
            TranslationUnit tu = new TranslationUnit(orgSegment, tgtSegment);
            results = tm.LanguageDirection.SearchTranslationUnit(GetSearchSettings(false, int.Parse(maxResults), int.Parse(minScore)), tu);
            return results;
        }
        /// <summary>
        /// Imports a single tmx file into specified file based TM
        ...
        /// </summary>
        /// <param name="tmPath"></param>
        public void CreateNewFileBasedTM(string tmPath, string slang, string tlang)
        {
            //create a new TM
            FileBasedTranslationMemory tm = new FileBasedTranslationMemory(tmPath,
                "A sample created as example of using TM API.",
                CultureInfo.GetCultureInfo(slang),
                CultureInfo.GetCultureInfo(tlang),
                GetFuzzyIndexes(),
                GetRecognizers(),
        ...
        private SearchSettings GetSearchSettings(bool target, int maxResults, int minScore)
        {
            SearchSettings settings = new SearchSettings();
            ...
            settings.AddPenalty(PenaltyType.FilterPenalty, 1);
