...
namespace Sdl.Enterprise2.Studio.Platform.Client.Discovery
{
	internal class DiscoveryInfo
	{
		...
		private readonly ILog log = LogManager.GetLogger(typeof(DiscoveryInfo));
		...
		private bool IsServiceAvaliable(ServiceEndpointInfo endpointInfo)
		{
			...
			{
				log.Error($"{address} Ping Failed", exception);
				return result;
			}
		}
		...
		private Uri GetServiceUri(string serviceName)
		{
			...
			log.InfoFormat("GetServiceUri({0}), preferredScheme({1}), discoveryScheme({2}), tokenType({3})", serviceName, preferredScheme, discoveryScheme, tokenType);
			...
			if (serviceEndpointInfo != null && IsServiceAvaliable(serviceEndpointInfo))
			{
				log.InfoFormat("1/Returning {0}", serviceEndpointInfo.Address);
				return serviceEndpointInfo.Address;
			}
			...
			if (serviceEndpointInfo != null && IsServiceAvaliable(serviceEndpointInfo))
			{
				log.InfoFormat("2/Returning {0}", serviceEndpointInfo.Address);
				return serviceEndpointInfo.Address;
			}
